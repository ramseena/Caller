...
StyleSheet,
Text,
View,DeviceEventEmitter ,ToastAndroid,
...
const Call= async (data) => {
  ...
  ToastAndroid.show('TEST ' + data, ToastAndroid.SHORT);
 
  if (data.state === "extra_state_ringing") {
  Toast.showWithGravity( "Ringingfgfg", Toast.LONG, Toast.BOTTOM);
  invokeApp();
  }
  else if (data.state === "extra_state_offhook") {
  Toast.showWithGravity("Call Started", Toast.LONG, Toast.BOTTOM)
  }
  else if (data.state === "extra_state_idle") {
  Toast.showWithGravity("Call Ended", Toast.LONG, Toast.BOTTOM)
  }
  }
...
componentDidMount()
{
DeviceEventEmitter.addListener('appInvoked', () => {
...
{this.state.incoming && (
<Text style={{fontSize: 50}}>PUHELU {this.state.number}{this.state.incoming}</Text>
)}
</View>
);
}
